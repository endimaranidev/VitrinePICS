...
app.post(`${P}/teams`, async (c) => {
  ...
  await kv.set(`team:${teamName}`, { teamName, description: description || "", logo: logo || "", createdAt: new Date().toISOString() });
...
app.put(`${P}/teams/:teamName`, async (c: any) => {
  ...
  await kv.set(`team:${teamName}`, { ...existing, description: description ?? existing.description, logo: logo ?? existing.logo });
  return c.json({ success: true });
});
